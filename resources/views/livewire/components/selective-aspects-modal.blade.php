...
<div>
    {{-- Modal Container with Alpine.js for instant display - using mary-ui modal --}}
    <div x-data="selectiveAspectsModal()" x-on:open-selection-modal-instant.window="openModal()"
        ...
        {{-- Use Mary UI modal component. Bind Livewire `show` so Livewire controls visibility. --}}
        <x-mary-modal wire:model="show" x-on:modal-loading.window="loading = true"
            x-on:modal-ready.window="loading = false; dataReady = true"
            box-class="relative w-full max-w-4xl max-h-[90vh] bg-white dark:bg-gray-800 rounded-lg shadow-xl transform transition-all flex flex-col">
