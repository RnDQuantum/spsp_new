<div class="w-full max-w-4xl mx-auto bg-white border border-slate-200 shadow-xl rounded-2xl p-8 md:p-12 print:border-none print:shadow-none">
    ...
    <!-- Section Header -->
    <div class="border-b border-slate-100 pb-6 mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h2 class="font-display text-2xl md:text-3xl text-slate-charcoal font-semibold">
                Riwayat <span class="text-forest-green italic">Karier</span>
            </h2>
        </div>
        <!-- Total Years Callout -->
        <div class="flex items-center gap-2">
            <span class="text-xs font-semibold text-slate-500">Masa Kerja Efektif:</span>
            <span class="text-sm font-bold text-slate-800 bg-slate-100 border border-slate-200 px-3 py-1 rounded-md">
                11 Tahun
            </span>
        </div>
    </div>
    ...
    <div class="relative pl-6 md:pl-8 border-l border-slate-200 space-y-12">
        @foreach ($timeline as $index => $item)
            <!-- Timeline Item -->
            <div class="relative">
                <!-- Bullet Marker -->
                <div class="absolute -left-[31px] md:-left-[39px] top-1.5 w-4 h-4 rounded-full bg-white border-4 border-forest-green shadow-sm flex items-center justify-center">
                    @if ($index === 0)
                        <!-- Active Pulse for Current Position -->
                        <span class="absolute w-6 h-6 rounded-full bg-forest-green/20 animate-ping"></span>
                    @endif
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div class="md:col-span-3 space-y-1">
                        <span class="inline-block text-xs font-bold text-forest-green bg-emerald-50 border border-emerald-100 px-2.5 py-0.5 rounded-full font-mono">
                            {{ $item['period'] }}
                        </span>
                        <p class="text-[11px] text-slate-400 font-semibold uppercase tracking-wider block mt-1">
                            {{ $item['unit'] }}
                        </p>
                    </div>
                    ...
                    <div class="md:col-span-9 space-y-3">
                        <h3 class="font-display font-semibold text-slate-charcoal text-lg">
                            {{ $item['role'] }}
                        </h3>
    ...
    <div class="mt-12 bg-slate-50 border border-slate-100 rounded-xl p-6 text-xs text-slate-500 leading-relaxed flex items-start gap-3">
        <i class="fas fa-circle-info text-forest-green text-sm mt-0.5"></i>
        <span>
