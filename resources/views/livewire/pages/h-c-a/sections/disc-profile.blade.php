<div class="w-full max-w-5xl mx-auto bg-white border border-warm-border rounded-xl p-8 md:p-12 print:border-none shadow-sm">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        @foreach ($quadrants as $quad)
            @if ($quad['isDominant'])
                ...
                <div class="bg-accent-amber/5 border-2 border-accent-amber rounded-xl p-6 relative shadow-xs transition-all duration-300 hover:scale-[1.01]">
    ...
    <!-- Section Bottom Info -->
    <div class="bg-warm-ivory border border-warm-border rounded-xl p-6 text-xs text-slate-600 leading-relaxed flex items-start gap-3">
        <i class="fas fa-circle-question text-accent-amber text-sm mt-0.5"></i>
        <span>
            <strong>Interpretasi Gaya Kerja ({{ $dominantStyle }}):</strong> {{ $interpretation }}
        </span>
    </div>
</div>
