...
1. **Evidence-Based HR & Psychometric Traceability (Level 1 Evidence Layer)**:
   * Menyediakan rincian data mentah dan matang (*standard scores*, Sten, T-Score, persentase subtes) dari setiap instrumen alat tes psikologi yang diikuti peserta.
   * Berfungsi sebagai basis pembuktian (*audit-proof*) ilmiah yang mendasari rating skala 1–5 pada Layer 1 (Kompetensi) dan Layer 2 (Potensi).
2. **Executive Journal Integrity (Separation of Concerns)**:
   * Menempatkan detail psikometri teknis di bagian lampiran (*Appendix*) di akhir laporan agar tidak mengaburkan fokus pembaca C-Level/Direksi yang membaca narasi strategis makro di halaman depan.
3. **Deep-Dive Diagnostic Support for Psychologists & Assessors**:
   * Memfasilitasi asesor, psikolog, dan komite talenta untuk melakukan verifikasi silang (*cross-validation*) terhadap dinamika kepribadian, ketahanan stres, dan kapasitas kognitif spesifik peserta.
...
* **Cakupan Instrumen Terisi (Sesuai `API_TEST_INSTRUMENTS_SCHEMA.md` & Norm Engine)**:
  1. **Kecerdasan / IQ**:
     * `A.1` & `A.2` — Typical CFIT 3A / 3B (Total IQ, Kategori, Skor Subtes 1–4, Interpretasi).
     * `A.5` — Typical IST (IQ Total, Kategori, 9 Subtes: SE, WA, AN, GE, RA, ZR, FA, WU, ME).
  2. **Kepribadian & Karakter**:
     ...
     * `B.2` — Typical 16PF (16 Faktor Sten Score, MDSten, WS, dan nilai 16 aspek kepribadian).
...
1. **Web Interactive View**:
   * Berada di **Klaster 7: Lampiran** pada sidebar navigasi TOC.
   * Tampilan modular berbasis sub-tab/pills per kategori alat tes (*Kognitif, Kepribadian, Sikap Kerja, Klinis/MMPI, Minat*).
   * Terdapat visualisasi grafik (Chart.js Bar/Radar untuk PAPI Kostik, 16PF, dan Kraepelin) serta tabel rekapitulasi subtes.
...
3. **Cross-Link Integrasi**:
   * Section 08 (IQ Kognitif), Section 09 (Kepribadian), Section 13 (EQ), dan Section 19 (Kesehatan Jiwa) memiliki *quick-jump button* menuju Section 24 untuk mempermudah navigasi pembuktian data.
